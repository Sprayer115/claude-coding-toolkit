...
name: vctk-challenge
user-invocable: true
description: Adversarial code review that grills the developer on their changes before approving. Use when user says 'challenge me', 'grill me on this', 'review my changes', 'challenge my code', 'devil\'s advocate review', or wants an interrogative code review. Forces developer to demonstrate mastery before merge.
metadata:
  author: Sprayer115
  version: "0.2.0"
  category: quality
  tags: [vctk, dev-workflow, review]
...
Before starting, surface the recommended workflow sequence:

```bash
ls .agent/specs/SPEC-*.md 2>/dev/null | head -1
```

If a spec exists, check whether /vctk-review-code has been run (look for an audit report). If not, warn:

> "Note: /vctk-review-code has not been run for this feature. The structured audit catches constraint and security issues that this challenge session does not. Recommend running /vctk-review-code first — or proceed here for a judgment-focused review."

Do NOT block. Proceed either way.
...
AskUserQuestion({
  questions: [{
    ...
    options: [
      ...
      { label: "Disagree — not relevant", description: "This doesn't apply to my change" }
    ],
    multiSelect: false
  }]
})
...
If "Disagree — not relevant" → ask for a one-line justification before accepting.

Accept the skip after justification. Log it in the scoring table as SKIPPED with reason.
...
|---|----------|----------|---------|
| 1 | [Short question] | [Summary of answer] | PASS / WEAK / FAIL / SKIPPED ([reason]) |
...
4. **DO NOT** ask questions you could answer by reading the code — ask about *judgment* and *trade-offs*
5. **DO NOT** suggest fixes during the grilling — save that for the verdict
6. **DO NOT** ask more than 12 questions total
