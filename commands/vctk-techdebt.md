...
If none of these files are found, do NOT silently continue. Ask the user to specify the stack:

```
AskUserQuestion({
  questions: [{
    question: "Could not detect a project manifest. What tech stack should I scan for?",
    header: "Stack",
    options: [
      { label: "PHP / Laravel", description: "Scan for PHP patterns (composer.json, Eloquent, Blade)" },
      { label: "JavaScript / TypeScript", description: "Scan for JS/TS patterns" },
      { label: "Python", description: "Scan for Python patterns" },
      { label: "Other / tell me", description: "I'll describe the stack in chat" }
    ],
    multiSelect: false
  }]
})
...
```
Grep: pattern="\bfunction\s+\w+|\bdef\s+\w+|\bconst\s+\w+"
```
...
Use the Glob tool to find source files, then Read them to check line counts. For example:

```
Glob: pattern="**/*.{ts,js,py,rs,go,php}" (exclude node_modules/, vendor/, dist/, build/, .git/)
```

For each matched file, note its line count. Flag files exceeding 300 lines, functions exceeding 50 lines, or nesting depth beyond 4 levels.
