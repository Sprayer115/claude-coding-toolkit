...
```bash
ls .agent/briefs/BRIEF-*.md 2>/dev/null
```

If no brief found → STOP immediately:
> "No feature brief found in .agent/briefs/. Run /vctk-feature-brief first."

If exactly one brief exists, read it and confirm. If multiple briefs exist, list them with their feature names and dates so the user can pick:

```
AskUserQuestion({
  questions: [{
    question: "Which feature brief should I design a spec for?",
    header: "Brief",
    options: [
      { label: "[BRIEF-name-1.md]", description: "feature: [feature name], created: [date]" },
      { label: "[BRIEF-name-2.md]", description: "feature: [feature name], created: [date]" }
    ],
    multiSelect: false
  }]
})
...
For a single brief, simply confirm:
...
```
WebSearch: "[technology] [pattern] best practices"
...
AskUserQuestion({
  questions: [{
    ...
    header: "Finalize",
    options: [
      { label: "Review decisions first", description: "Show me all decisions before generating" },
...
After generating the spec, ask the user to explicitly approve it before saving with `APPROVED FOR IMPLEMENTATION` status:

```
AskUserQuestion({
  questions: [{
    question: "Spec generated. Approve it for implementation?",
    header: "Approve",
    options: [
      { label: "Approve for implementation", description: "Set status to APPROVED FOR IMPLEMENTATION and save" },
      { label: "Save as draft", description: "Save with PENDING APPROVAL status — not yet ready to build" },
      { label: "Revise first", description: "Go back and change a decision" }
    ],
    multiSelect: false
  }]
})
...
Set `status` based on the user's choice. Never auto-set `APPROVED FOR IMPLEMENTATION`.

```markdown
---
status: [APPROVED FOR IMPLEMENTATION | PENDING APPROVAL]
author: [User Name]
created: [YYYY-MM-DD]
