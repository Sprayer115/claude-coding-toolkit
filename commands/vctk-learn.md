...
Read the project's `CLAUDE.md` using the Read tool:

```
Read: file_path="CLAUDE.md"
```

If the file does not exist, note that we'll create one.

Also read the global CLAUDE.md. The path is platform-dependent:
- **Windows:** `%USERPROFILE%\.claude\CLAUDE.md` (detect via `$USERPROFILE` env var)
- **Unix/macOS:** `~/.claude/CLAUDE.md`
...
Then use the Read tool on the resolved path. If it does not exist, note it.
...
Always show lessons before saving — never add rules without explicit per-lesson approval.

If "Let me pick" → list each lesson and ask the user to confirm or reject individually.
